...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        ...
        const [res, pat, inc, gst, msg] = await Promise.all([
  ...
  const EmptyState = ({ label }: { label: string }) => (
    <div className="py-24 text-center">
      <Ghost size={64} className="mx-auto text-slate-200 mb-6 opacity-40" />
      <p className="font-black text-slate-300 uppercase tracking-[0.3em] italic text-xs">Data {label} Belum Tersedia</p>
    </div>
  );
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (isValid) {
      ...
      setLoginError(isAdmin ? 'PIN Admin Salah (admin123)' : 'PIN Petugas Salah (1234)');
    }
  };
  ...
  const handleSaveResident = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Operasi Gagal: Pastikan koneksi cloud aktif.");
    }
  };
  ...
  const chartData = [{ name: 'Patroli', val: patrolLogs.length }, { name: 'Tamu', val: guests.length }, { name: 'Insiden', val: incidents.length }];
  ...
  if (!currentUser) {
    ...
    return (
      <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-[950px] flex flex-col md:flex-row rounded-[3rem] shadow-2xl overflow-hidden">
          <div className="w-full md:w-5/12 bg-slate-900 p-8 lg:p-12 text-white flex flex-col justify-between">
            <div>
              ...
              <h1 className="text-3xl lg:text-4xl font-black mb-4 tracking-tighter italic uppercase leading-none">TKA SECURE <br/><span className="text-amber-500 not-italic text-2xl font-light">Cloud Master</span></h1>
              <p className="text-slate-400 text-sm italic leading-relaxed">Pusat data keamanan real-time untuk kenyamanan penghuni.</p>
            </div>
            <div className="p-4 bg-white/5 rounded-2xl border border-white/10 flex items-center gap-3">
               <div className={`w-2 h-2 rounded-full animate-pulse ${cloudStatus === 'connected' ? 'bg-green-500' : 'bg-amber-500'}`}></div>
               <span className="text-[10px] font-black uppercase tracking-widest text-slate-300 italic">{cloudStatus === 'connected' ? 'Connected' : 'Connecting...'}</span>
            </div>
          </div>
          ...
          <div className="w-full md:w-7/12 p-8 lg:p-14 h-[650px] lg:h-[750px] flex flex-col bg-white overflow-y-auto no-scrollbar">
            ...
            <div className="relative mb-6">
              ...
              <input type="text" placeholder={`Cari nama ${loginTab}...`} className="w-full pl-14 pr-6 py-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-amber-500 outline-none font-bold text-sm transition-all" value={loginSearch} onChange={e => setLoginSearch(e.target.value)} />
            </div>
              ...
              {filtered.map((u: any) => (
                <button key={u.id} type="button" onClick={() => { setSelectedUser(u); setLoginError(''); }}
                  ...
                  <div className="flex-1 text-left">
                    <p className="font-black text-slate-900 text-base uppercase">{u.name}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{u.sub || 'Identity Verified'}</p>
                  </div>
                </button>
              ))}
            </div>
  ...
  return (
    <Layout user={currentUser} onLogout={() => setCurrentUser(null)} activeTab={activeTab} setActiveTab={setActiveTab}>
        ...
        <div className="space-y-8 animate-slide-up pb-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {[
              ...
              { label: 'Tamu Terdeteksi', val: stats.guests, icon: <Users size={24}/>, color: 'amber' },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-900 text-white p-8 lg:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[300px]">
               <div className="relative z-10">
                  ...
                  <p className="text-slate-400 text-sm italic leading-relaxed font-medium">"{securityBriefing || 'Menghubungkan asisten AI...'}"</p>
               </div>
               <button onClick={() => setActiveTab('chat')} className="w-full bg-amber-500 text-slate-900 font-black py-5 rounded-2xl text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all mt-10">HUBUNGI TIM <ArrowRight size={20}/></button>
            </div>
          </div>
        </div>
      )}

      {/* 2. LOG RESIDENT / STATUS HUNIAN */}
      {(activeTab === 'log_resident') && (
        <div className="space-y-8 animate-slide-up pb-24">
           <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase italic">Monitor Kehadiran Unit</h3>
      ...
      {activeTab === 'reports' && (
        <div className="space-y-8 animate-slide-up pb-20">
           <h3 className="text-2xl font-black text-slate-900 uppercase italic">Cloud Activity Feed</h3>
           <div className="bg-white p-6 lg:p-12 rounded-[3rem] shadow-sm border border-slate-100">
              <div className="space-y-12">
                {timeline.length > 0 ? timeline.map((item: any, idx) => (
                  <div key={idx} className="flex gap-6 lg:gap-10 group animate-slide-up">
                     ...
                     <div className="flex-1 pb-12 border-b border-slate-50 last:border-none">
                        <div className="flex justify-between items-center mb-3">
                           <h4 className="font-black text-slate-900 text-base uppercase italic">{item.type === 'PATROL' ? `Patroli: ${item.checkpoint}` : item.type === 'INCIDENT' ? `Insiden: ${item.type}` : `Pendaftaran Tamu`}</h4>
                        ...
                        <p className="text-sm lg:text-base text-slate-500 font-medium mb-6 italic leading-relaxed">
                          {item.type === 'PATROL' ? `Area dipastikan ${item.status}. ${item.note || ''}` : item.description || `Kedatangan tamu ${item.name} menuju unit ${item.visitToName}.`}
                        </p>
      ...
      {activeTab === 'incident' && (
        <div className="space-y-8 animate-slide-up pb-20">
           <div className="flex justify-between items-center">
              <h3 className="text-2xl font-black text-slate-900 uppercase italic">Emergency Log</h3>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {incidents.length > 0 ? incidents.map(inc => (
                <div key={inc.id} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all">
                   ...
                   <h4 className="text-xl font-black text-slate-900 mb-2 italic uppercase">{inc.type}</h4>
                   <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2 italic"><MapPin size={14}/> {inc.location}</p>
                   <p className="text-sm text-slate-600 font-medium leading-relaxed italic border-l-4 border-slate-100 pl-4">"{inc.description}"</p>
                </div>
              )) : <EmptyState label="Insiden" />}
           </div>
        </div>
      )}
      ...
      {activeTab === 'guests' && (
        <div className="space-y-8 animate-slide-up pb-24">
           <div className="flex justify-between items-center">
              ...
              <button onClick={() => setIsModalOpen('GUEST')} className="bg-blue-600 text-white px-8 py-4 rounded-[1.5rem] font-black text-[10px] uppercase flex items-center gap-3 shadow-2xl active:scale-95 transition-all"><UserPlus size={20}/> DAFTAR TAMU</button>
           </div>
           <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden overflow-x-auto no-scrollbar">
              <table className="w-full text-left min-w-[800px]">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400">Nama Tamu</th>
                    <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400">Tujuan</th>
                    <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400">Waktu</th>
                    <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400 text-right">Aksi</th>
                  </tr>
                </thead>
                   ...
                   {guests.length > 0 ? guests.map(g => (
                     <tr key={g.id} className="hover:bg-slate-50/50 transition-colors">
                       <td className="px-10 py-8">
                          <p className="font-black text-slate-900 text-base uppercase leading-none">{g.name}</p>
                          <p className="text-[11px] text-slate-400 font-medium italic mt-1">"{g.purpose}"</p>
                       </td>
                       <td className="px-10 py-8 text-xs font-black text-slate-500 uppercase">{g.visitToName}</td>
                       <td className="px-10 py-8 text-[11px] font-black text-slate-400">{new Date(g.entryTime).toLocaleString('id-ID')}</td>
                       <td className="px-10 py-8 text-right">
                          <button onClick={() => db.guest.update(g.id, { status: g.status === 'IN' ? 'OUT' : 'IN' })} className={`px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all ${g.status === 'IN' ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'}`}>{g.status === 'IN' ? 'CHECK-OUT' : 'MASUK'}</button>
                       </td>
                     </tr>
                   )) : <tr><td colSpan={4}><EmptyState label="Tamu" /></td></tr>}
                </tbody>
              </table>
           </div>
        </div>
      ...
      {/* 7. RESIDENTS / MASTER DATABASE */}
      {activeTab === 'residents' && (
        <div className="space-y-8 animate-slide-up pb-24">
           <div className="flex justify-between items-center">
              <h3 className="text-2xl font-black text-slate-900 uppercase italic">Master Database Warga</h3>
              {currentUser.role === 'ADMIN' && <button onClick={() => { setEditingItem(null); setResForm({ name: '', houseNumber: '', block: BLOCKS[0], phoneNumber: '', isHome: true }); setIsModalOpen('RESIDENT'); }} className="bg-slate-900 text-white p-4 rounded-2xl shadow-xl active:scale-95"><Plus size={24}/></button>}
           </div>
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {residents.length > 0 ? residents.map(res => (
                <div key={res.id} className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col justify-between group hover:shadow-xl transition-all">
                   <div>
                   ...
                   <div className="flex gap-2">
                      ...
                      {currentUser.role === 'ADMIN' && <button onClick={() => { setEditingItem(res); setResForm(res); setIsModalOpen('RESIDENT'); }} className="p-4 bg-slate-100 text-slate-400 hover:text-blue-500 rounded-2xl transition-all"><Edit2 size={16}/></button>}
                   </div>
                </div>
              )) : <EmptyState label="Warga" />}
           </div>
        </div>
      )}
      ...
      {activeTab === 'chat' && (
        <div className="max-w-4xl mx-auto h-[calc(100vh-220px)] flex flex-col animate-slide-up pb-10">
                ...
                <div key={msg.id} className={`flex ${msg.senderId === currentUser.id ? 'justify-end' : 'justify-start'} animate-slide-up`}>
                   <div className={`max-w-[85%] p-6 rounded-[2.5rem] relative shadow-sm ${msg.senderId === currentUser.id ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-slate-50 text-slate-900 rounded-tl-none border border-slate-100'}`}>
                      <div className="flex justify-between items-center gap-6 mb-2">
                         ...
                         <span className="text-[8px] font-bold px-2 py-0.5 bg-white/10 rounded-full uppercase opacity-40">{msg.senderRole}</span>
                      </div>
           ...
           <form onSubmit={handleSendChat} className="bg-white p-6 rounded-b-[3rem] border-t border-slate-100 flex gap-4 shadow-2xl items-center sticky bottom-0">
              <input type="text" placeholder="Tulis instruksi atau pesan..." className="flex-1 px-8 py-5 rounded-[2rem] bg-slate-50 border-2 border-transparent outline-none font-bold text-sm lg:text-base focus:border-amber-500 transition-all shadow-inner" value={chatInput} onChange={e => setChatInput(e.target.value)} />
      ...
      {activeTab === 'settings' && (
        <div className="max-w-2xl mx-auto space-y-8 animate-slide-up pb-20">
           <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 text-center">
              <div className="w-32 h-32 bg-slate-900 text-amber-500 rounded-[3rem] flex items-center justify-center font-black text-5xl mx-auto mb-8 shadow-2xl border-4 border-white">
                {currentUser.name.charAt(0)}
              </div>
              <h3 className="text-3xl font-black text-slate-900 uppercase italic mb-1 leading-none">{currentUser.name}</h3>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-10 italic">Akses Level: {currentUser.role}</p>
              
              <div className="grid grid-cols-1 gap-4">
                 <button onClick={() => setCurrentUser(null)} className="w-full py-5 bg-red-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all"><LogOut size={20}/> KELUAR DARI SISTEM</button>
                 <button className="w-full py-5 bg-slate-50 text-slate-400 rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3"><Settings size={18}/> PENGATURAN LANJUTAN</button>
              </div>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl animate-slide-up overflow-hidden">
            ...
            <form onSubmit={handleSaveResident} className="p-10 space-y-6">
              ...
              <button type="submit" className="w-full py-5 bg-slate-900 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-2xl active:scale-95 transition-all">SIMPAN PERUBAHAN</button>
            </form>
          </div>
        </div>
      )}

      {/* MODAL: PATROL REPORT (WITH CAMERA) */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl animate-slide-up overflow-hidden">
            <div className={`p-8 text-white flex justify-between items-center ${patrolAction.status === 'OK' ? 'bg-green-600' : 'bg-red-600'}`}>
              <div>
                <h3 className="text-xl font-black uppercase italic">{patrolAction.cp}</h3>
                <p className="text-[10px] font-black uppercase opacity-70 mt-1 tracking-widest">Digital Field Log</p>
              </div>
              <button onClick={() => setIsModalOpen(null)}><X size={28}/></button>
            </div>
            <form onSubmit={async (e) => {
              ...
              <div className="space-y-2">
                ...
                <textarea required placeholder="Deskripsi kondisi..." className="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 border-2 border-transparent outline-none font-bold text-sm min-h-[140px] shadow-inner focus:border-slate-900 transition-all" value={patrolReportData.note} onChange={e => setPatrolReportData({...patrolReportData, note: e.target.value})} />
              </div>
      ...
      {/* MODAL: GUEST & INCIDENT (SAME LOGIC AS ABOVE - EXCLUDED FOR BREVITY BUT FULLY FUNCTIONAL) */}
      {isModalOpen === 'GUEST' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl animate-slide-up overflow-hidden">
            <div className="p-8 bg-blue-600 text-white flex justify-between items-center">
              <h3 className="text-2xl font-black uppercase italic leading-none">Registrasi Tamu</h3>
              <button onClick={() => setIsModalOpen(null)}><X size={28}/></button>
            </div>
            <form onSubmit={async (e) => {
               e.preventDefault();
               ...
               await db.guest.create({ id: `g-${Date.now()}`, name: guestForm.name, visitToId: guestForm.visitToId, visitToName: target ? `${target.name} (${target.block}-${target.houseNumber})` : 'Unit', purpose: guestForm.purpose || 'Kunjungan', entryTime: new Date().toISOString(), status: 'IN' });
               setIsModalOpen(null);
               setGuestForm({ name: '', visitToId: '', purpose: '' });
            }} className="p-8 space-y-6">
              <input type="text" required placeholder="Nama Tamu..." className="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 border-2 border-transparent focus:border-blue-600 outline-none font-bold text-sm shadow-inner transition-all" value={guestForm.name} onChange={e => setGuestForm({...guestForm, name: e.target.value})} />
              <select required className="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 outline-none font-bold text-sm" value={guestForm.visitToId} onChange={e => setGuestForm({...guestForm, visitToId: e.target.value})}>
                 <option value="">-- Tujuan Unit --</option>
                 {residents.map(r => <option key={r.id} value={r.id}>{r.block}-{r.houseNumber} ({r.name})</option>)}
              </select>
              <textarea required placeholder="Tujuan..." className="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 outline-none font-bold text-sm min-h-[140px] shadow-inner" value={guestForm.purpose} onChange={e => setGuestForm({...guestForm, purpose: e.target.value})} />
              <button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-widest active:scale-95 shadow-2xl transition-all">DAFTARKAN TAMU MASUK</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl animate-slide-up overflow-hidden">
            <div className="p-8 bg-red-600 text-white flex justify-between items-center">
              <h3 className="text-2xl font-black uppercase italic leading-none">Lapor Darurat</h3>
              <button onClick={() => setIsModalOpen(null)}><X size={28}/></button>
            </div>
            <form onSubmit={async (e) => {
               e.preventDefault();
               if (!currentUser) return;
               await db.incident.create({ ...incForm, id: `inc-${Date.now()}`, reporterId: currentUser.id, reporterName: currentUser.name, timestamp: new Date().toISOString(), status: 'PENDING' });
               setIsModalOpen(null);
               setIncForm({ type: 'Pencurian', location: '', description: '', severity: 'MEDIUM' });
            }} className="p-8 space-y-6">
              <select required className="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 border-2 border-transparent focus:border-red-600 outline-none font-bold text-sm shadow-inner" value={incForm.type} onChange={e => setIncForm({...incForm, type: e.target.value})}>
                 <option value="Pencurian">Pencurian</option><option value="Kebakaran">Kebakaran</option><option value="Kriminalitas">Kriminalitas</option><option value="Lainnya">Lainnya</option>
              </select>
              <input type="text" required placeholder="Lokasi..." className="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 border-2 border-transparent focus:border-red-600 outline-none font-bold text-sm shadow-inner" value={incForm.location} onChange={e => setIncForm({...incForm, location: e.target.value})} />
              <textarea required placeholder="Deskripsi..." className="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 border-2 border-transparent focus:border-red-600 outline-none font-bold text-sm min-h-[140px] shadow-inner" value={incForm.description} onChange={e => setIncForm({...incForm, description: e.target.value})} />
              <button type="submit" className="w-full py-5 bg-red-600 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-widest active:scale-95 shadow-2xl">KIRIM LAPORAN SEKARANG</button>
            </form>
          </div>
        </div>
      )}
