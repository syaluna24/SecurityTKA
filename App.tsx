...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const rSub = db.resident.subscribe((p) => {
    ...
    const iSub = db.incident.subscribe((p) => {
    ...
    const cSub = db.chat.subscribe((p) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (isValid) {
      ...
      setLoginError(isAdmin ? 'PIN Admin Salah' : 'PIN Petugas Salah');
    }
  };
  ...
  const saveResident = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Gagal menyimpan data warga ke database Prisma.");
    }
  };
  ...
  const savePatrol = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      alert("Gagal menyimpan data patroli.");
    }
  };
  ...
  const timeline = useMemo(() => {
    const combined = [
      ...patrolLogs.map(l => ({ ...l, type: 'PATROL', sort: l.timestamp })),
      ...incidents.map(i => ({ ...i, type: 'INCIDENT', sort: i.timestamp })),
      ...guests.map(g => ({ ...g, type: 'GUEST', sort: g.entryTime }))
    ];
    return combined.sort((a, b) => new Date(b.sort).getTime() - new Date(a.sort).getTime()).slice(0, 20);
        ...
        <div className="bg-white w-full max-w-[950px] flex flex-col md:flex-row rounded-[2.5rem] shadow-2xl overflow-hidden">
          <div className="w-full md:w-5/12 bg-slate-900 p-8 lg:p-12 text-white flex flex-col justify-between relative">
            <div>
              ...
              <h1 className="text-3xl lg:text-4xl font-black mb-4 tracking-tighter italic uppercase leading-none">TKA SECURE <br/><span className="text-amber-500 not-italic text-2xl font-light">Prisma Edition</span></h1>
              <p className="text-slate-400 text-sm italic leading-relaxed">Sistem keamanan terpadu dengan basis data cloud real-time.</p>
            </div>
          ...
          <div className="w-full md:w-7/12 p-8 lg:p-14 h-[650px] lg:h-[750px] flex flex-col bg-white overflow-y-auto no-scrollbar">
            ...
            {selectedUser && (
              <form onSubmit={handleLogin} className="pt-8 border-t border-slate-100 space-y-5">
                <input type="password" required placeholder="PIN (1234 / admin123)"
        ...
        <div className="space-y-8 animate-slide-up pb-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {[
              ...
              { label: 'Tamu In-Area', val: stats.guestsIn, icon: <Users size={24}/>, color: 'amber' },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-white p-6 lg:p-10 rounded-[2.5rem] shadow-sm border border-slate-100 min-h-[400px]">
               <h3 className="text-xl font-black text-slate-900 mb-10 flex items-center gap-4 uppercase italic leading-none"><Activity size={24} className="text-amber-500 animate-pulse"/> Analisis Data</h3>
            ...
            <div className="bg-slate-900 text-white p-8 lg:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[300px]">
               <div className="relative z-10">
                  ...
                  <p className="text-slate-400 text-sm italic leading-relaxed font-medium">"{securityBriefing || 'Memuat briefing terbaru...'}"</p>
               </div>
               <button onClick={() => setActiveTab('chat')} className="w-full bg-amber-500 text-slate-900 font-black py-5 rounded-2xl text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all mt-10">BUKA CHAT TIM <ArrowRight size={20}/></button>
            </div>
          </div>
        </div>
      )}
      ...
      {(activeTab === 'residents' || activeTab === 'log_resident') && (
        <div className="space-y-8 animate-slide-up pb-24">
           <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase italic">{activeTab === 'residents' ? 'Database Unit' : 'Monitor Kehadiran'}</h3>
              <div className="flex gap-4 items-center">
                <div className="relative w-full lg:w-[450px]">
                  <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300" size={20}/>
                  <input type="text" placeholder="Cari unit atau penghuni..." className="w-full pl-14 pr-6 py-4 rounded-[1.8rem] bg-white border border-slate-100 outline-none focus:border-amber-500 font-bold text-sm shadow-sm" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
              ...
              {residents.filter(r => r.name.toLowerCase().includes(searchQuery.toLowerCase()) || r.houseNumber.includes(searchQuery)).map(res => (
                <div key={res.id} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col justify-between hover:shadow-xl transition-all group">
                   <div className="flex justify-between items-start mb-6">
                      ...
                      <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${res.isHome ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>{res.isHome ? 'ADA DI UNIT' : 'KELUAR'}</span>
                   </div>
                   <div className="mb-8">
                      <h4 className="font-black text-slate-900 uppercase italic truncate text-base mb-1 leading-none">{res.name}</h4>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Unit: <span className="text-slate-900">{res.block}-{res.houseNumber}</span></p>
                   </div>
                   {activeTab === 'log_resident' ? (
                     <button onClick={() => db.resident.update(res.id, { isHome: !res.isHome })} className={`w-full py-4 rounded-[1.5rem] font-black text-[10px] uppercase flex items-center justify-center gap-3 active:scale-95 transition-all shadow-lg ${res.isHome ? 'bg-slate-900 text-white' : 'bg-green-500 text-white'}`}>
           ...
           <div className="flex-1 bg-white rounded-t-[3rem] shadow-sm border border-slate-100 overflow-y-auto p-6 lg:p-10 space-y-8 no-scrollbar relative">
              {chatMessages.length > 0 ? chatMessages.map((msg, i) => (
                ...
                <div className="h-full flex flex-col items-center justify-center text-slate-200 italic py-24">
                   ...
                   <p className="font-black uppercase tracking-[0.3em] text-[10px]">Pusat Koordinasi Keamanan</p>
                </div>
              )}
              <div ref={chatEndRef} />
           </div>
           <form onSubmit={postChat} className="bg-white p-6 lg:p-8 rounded-b-[3rem] border-t border-slate-100 flex gap-4 shadow-xl items-center">
              <input type="text" placeholder="Ketik pesan atau instruksi..." className="flex-1 px-8 py-5 rounded-[1.8rem] bg-slate-50 border-2 border-transparent outline-none font-bold text-sm lg:text-base focus:border-amber-500 transition-all shadow-inner" value={chatInput} onChange={e => setChatInput(e.target.value)} />
              ...
              <div className="space-y-12">
                {timeline.length > 0 ? timeline.map((item: any, idx) => (
                  <div key={idx} className="flex gap-6 lg:gap-10 group">
                     ...
                     <div className="flex-1 pb-12 border-b border-slate-50 last:border-none">
                        <div className="flex justify-between items-center mb-3">
                           <h4 className="font-black text-slate-900 text-base lg:text-lg uppercase italic">{item.type === 'PATROL' ? `Patroli: ${item.checkpoint}` : item.type === 'INCIDENT' ? `Insiden: ${item.type}` : `Akses Tamu`}</h4>
                           <span className="text-[11px] font-black text-slate-300 uppercase tracking-widest">{new Date(item.sort).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        </div>
                        <p className="text-sm lg:text-base text-slate-500 font-medium mb-6 italic leading-relaxed">
                          {item.type === 'PATROL' ? `Status ${item.status}. ${item.note || ''}` : item.description || `Pendaftaran tamu ${item.name} menuju ${item.visitToName}.`}
                        </p>
                        {item.photo && <img src={item.photo} alt="Evidence" className="mb-6 rounded-[2rem] w-full max-w-md border border-slate-100 shadow-xl" />}
                        <div className="flex flex-wrap gap-3">
                           <span className={`px-4 lg:px-5 py-2 lg:py-2.5 rounded-full text-[9px] font-black uppercase tracking-widest ${item.status === 'OK' || item.status === 'RESOLVED' || item.status === 'IN' ? 'bg-green-50 text-green-600 shadow-sm' : 'bg-red-50 text-red-600 shadow-sm'}`}>{item.status || 'REPORTED'}</span>
                  ...
                  <div className="py-24 text-center">
                     ...
                     <p className="font-black text-slate-200 uppercase tracking-widest italic">Belum Ada Riwayat Hari Ini</p>
                  </div>
                )}
              </div>
           </div>
      ...
      {activeTab === 'guests' && (
        <div className="space-y-8 animate-slide-up pb-24">
           <div className="flex justify-between items-center">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase italic">Log Tamu Perumahan</h3>
              <button onClick={() => setIsModalOpen('GUEST')} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase flex items-center gap-3 shadow-2xl active:scale-95 transition-all"><UserPlus size={20}/> REGISTRASI TAMU</button>
           </div>
                   ...
                   {guests.map(g => (
                     <tr key={g.id} className="hover:bg-slate-50/50 transition-colors">
                       <td className="px-10 py-8">
                          <p className="font-black text-slate-900 text-base mb-1 italic uppercase leading-none">{g.name}</p>
          ...
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl animate-slide-up overflow-hidden">
            ...
            <form onSubmit={saveResident} className="p-10 space-y-6">
              ...
              <div className="space-y-1">
                ...
                <input type="text" required placeholder="081234..." className="w-full px-8 py-5 rounded-2xl bg-slate-50 outline-none font-bold text-sm border-2 border-slate-100 focus:border-slate-900 shadow-inner" value={resForm.phoneNumber} onChange={e => setResForm({...resForm, phoneNumber: e.target.value})} />
              </div>
      ...
      {isModalOpen === 'GUEST' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl animate-slide-up overflow-hidden">
            ...
            <form onSubmit={saveGuest} className="p-8 lg:p-12 space-y-6">
      ...
      {isModalOpen === 'INCIDENT' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl animate-slide-up overflow-hidden">
            ...
            <form onSubmit={saveIncident} className="p-8 lg:p-12 space-y-6">
      ...
      {isModalOpen === 'PATROL_REPORT' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl animate-slide-up overflow-hidden">
            ...
            <form onSubmit={savePatrol} className="p-8 lg:p-12 space-y-6 lg:space-y-8">
